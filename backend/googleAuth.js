...
export async function getOAuthClient() {
  try {
    ...
    if (!client_id || !client_secret || !redirectUri) {
      throw new Error('Missing required Google OAuth environment variables. Check .env file.');
    }

    console.log(`Using redirect URI from env: ${redirectUri}`);
    const oAuth2Client = new google.auth.OAuth2(client_id, client_secret, redirectUri);
    return oAuth2Client;
  } catch (err) {
      console.error('Error initializing OAuth client:', err);
      throw new Error('Could not initialize OAuth client: ' + err.message);
  }
}
